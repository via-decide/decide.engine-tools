...
const key = 'orchard.wave1.orchard-profile-builder';

function run() {
  const payload = EngineUtils.tryParse(payloadEl.value.trim());
  const mode = modeEl.value;
  const result = EngineModels.orchardProfileBuilder(payload, mode);
...
document.getElementById('download').addEventListener('click', () => EngineUtils.downloadText('orchard-profile-builder-output.json', outputEl.textContent || ''));
...
if (saved) {
  ...
  payloadEl.value = JSON.stringify(EngineModels.defaults['orchardProfileBuilder'], null, 2);
}
