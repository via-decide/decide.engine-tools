...
const key = 'orchard.wave1.weekly-harvest-engine';

function run() {
  const payload = EngineUtils.tryParse(payloadEl.value.trim());
  const mode = modeEl.value;
  const result = EngineModels.weeklyHarvestEngine(payload, mode);
...
document.getElementById('download').addEventListener('click', () => EngineUtils.downloadText('weekly-harvest-engine-output.json', outputEl.textContent || ''));
...
if (saved) {
  ...
  payloadEl.value = JSON.stringify(EngineModels.defaults['weeklyHarvestEngine'], null, 2);
}
