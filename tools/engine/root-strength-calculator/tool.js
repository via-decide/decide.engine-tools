...
const key = 'orchard.wave1.root-strength-calculator';

function run() {
  const payload = EngineUtils.tryParse(payloadEl.value.trim());
  const mode = modeEl.value;
  const result = EngineModels.rootStrengthCalculator(payload, mode);
...
document.getElementById('download').addEventListener('click', () => EngineUtils.downloadText('root-strength-calculator-output.json', outputEl.textContent || ''));
...
if (saved) {
  ...
  payloadEl.value = JSON.stringify(EngineModels.defaults['rootStrengthCalculator'], null, 2);
}
