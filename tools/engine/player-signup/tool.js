...
const key = 'orchard.wave1.player-signup';

function run() {
  const payload = EngineUtils.tryParse(payloadEl.value.trim());
  const mode = modeEl.value;
  const result = EngineModels.playerSignup(payload, mode);
...
document.getElementById('download').addEventListener('click', () => EngineUtils.downloadText('player-signup-output.json', outputEl.textContent || ''));
...
if (saved) {
  ...
  payloadEl.value = JSON.stringify(EngineModels.defaults['playerSignup'], null, 2);
}
