...
const key = 'orchard.wave1.thirty-day-promotion-engine';

function run() {
  const payload = EngineUtils.tryParse(payloadEl.value.trim());
  const mode = modeEl.value;
  const result = EngineModels.thirtyDayPromotionEngine(payload, mode);
...
document.getElementById('download').addEventListener('click', () => EngineUtils.downloadText('thirty-day-promotion-engine-output.json', outputEl.textContent || ''));
...
if (saved) {
  ...
  payloadEl.value = JSON.stringify(EngineModels.defaults['thirtyDayPromotionEngine'], null, 2);
}
