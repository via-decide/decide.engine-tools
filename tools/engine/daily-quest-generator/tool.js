...
const key = 'orchard.wave1.daily-quest-generator';

function run() {
  const payload = EngineUtils.tryParse(payloadEl.value.trim());
  const mode = modeEl.value;
  const result = EngineModels.dailyQuestGenerator(payload, mode);
...
document.getElementById('download').addEventListener('click', () => EngineUtils.downloadText('daily-quest-generator-output.json', outputEl.textContent || ''));
...
if (saved) {
  ...
  payloadEl.value = JSON.stringify(EngineModels.defaults['dailyQuestGenerator'], null, 2);
}
