...
### Fixed
- Fixed `tests/unit/agent-manager.test.js` export ordering so late regression assertions are included in `passed`/`failed` totals consumed by `tests/unit/run.js`, preventing false-green unit runs when those checks fail.
- Fixed `core/agent-manager.js` lifecycle error handling so scheduler-captured failures in `init`, `run`, and `dispose` are propagated as thrown run failures and correctly mark agent trace flows as failed instead of silently succeeding.
