...
### Fixed
- Fixed plugin trace propagation so `PluginSystem.executePlugin` uses the caller-provided trace engine/context for existing flow IDs, preventing `Unknown flow` failures during cross-runtime tracing.
