...
### Fixed
- Fixed `core/agent-manager.js` lifecycle state-keying and scheduler failure handling so each `runAgent` invocation uses an isolated state-machine key, repeated runs of the same agent no longer fail with stale `COMPLETED` state, and failed scheduled `agent.run` tasks emit `fail` (not `complete`) transitions.
