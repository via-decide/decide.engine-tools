...
class PluginSystem {
  ...
  executePlugin(pluginId, input, traceContext = null) {
    ...
    const trace = traceContext && traceContext.trace ? traceContext.trace : this.trace;
    const parentFlowId = traceContext && traceContext.flowId ? traceContext.flowId : trace.startFlow({ source: 'plugin-system', pluginId });
    const ownFlow = !(traceContext && traceContext.flowId);
    const spanId = trace.startSpan(parentFlowId, { name: `plugin.execute:${pluginId}`, parentId: traceContext && traceContext.parentSpanId ? traceContext.parentSpanId : null });

    try {
      const ctx = this.createSandboxContext(pluginId, entry.state);
      const result = entry.plugin.execute(ctx, input);
      this.log('execute', pluginId, { ok: true });
      trace.endSpan(spanId, { pluginId });
      if (ownFlow) trace.endFlow(parentFlowId, { failed: false, pluginId });
      return { ok: true, result };
    } catch (error) {
      this.log('execute', pluginId, { ok: false, error: error.message });
      trace.fail(spanId, error, { pluginId });
      if (ownFlow) trace.endFlow(parentFlowId, { failed: true, pluginId, error: error.message });
