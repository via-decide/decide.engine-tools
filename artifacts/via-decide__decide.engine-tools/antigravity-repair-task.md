...
TASK
Build a global GlassModalController for dynamic context injection. 1. Create shared/glass-modal.js. 2. Implement a globally accessible GlassModal.open(title, contentHTML, actionButtons) method. 3. The method must target (or inject) a .modal.panel-glass container overlaying the grid background. 4. It should inject the title (using the Syne font stack), the contentHTML (using DM Mono), and render the actionButtons. 5. If an action button has type: 'critical', apply the .btn-glitch CSS class to it automatically. 6. Add event listeners to close the modal on an outside click or 'Escape' key press, fading out the .panel-glass smoothly.
