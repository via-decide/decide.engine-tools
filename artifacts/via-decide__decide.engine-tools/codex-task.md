...
CONSTRAINTS
Do NOT use any external UI or component libraries. Use Vanilla JS and the provided shared engine styles. The moderation chain must be implemented using a Publish-Subscribe (pub/sub) event model so that new security filters can be toggled on or off in the UI without rewriting the core execution loop.
