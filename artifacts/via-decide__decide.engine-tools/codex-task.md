...
MISSION
Build a new micro-frontend named 'NexusGraph' (The Temporal Memory Visualizer). Create a new folder /NexusGraph and a single index.html file inside it. CORE LOGIC (Pure Vanilla JS): 1. Setup & Memory Load: - A drag-and-drop zone to load the .vdb (Vector Database) file. 2. The Rendering Engine (HTML5 Canvas): - Extract the first 500 commit objects (for performance) from the VDB. - Implement a basic 2D Force-Directed Graph algorithm in vanilla JS.

CONSTRAINTS
Zero NPM dependencies (NO D3.js or Three.js). Pure Vanilla JS and raw HTML5 <canvas> API. The force-directed graph calculation should be optimized so it doesn't freeze the main thread (use requestAnimationFrame).
